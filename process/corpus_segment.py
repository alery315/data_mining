...
# 保存至文件
def savefile(save_path, content):
    # print(content)
...
def split_word_by_cate(mydir, corpus_path, seg_path):
    ...
    for file_path in file_list:
        ...
        for line in content:
            ...
            # 这里去除停用词
            stop_words = read_stop_word()  # 这里把停用词存到了一个dict中,O(1)的存在访问,可以快一些
...
def split_train_test(train_path_, test_path_, remaining=-1):
    if remaining == -1:
        cate_num_dict = {}
        dirs = os.listdir(train_path_)
        for my_dir in dirs:
        ...
        dirs = os.listdir(train_path_)
        for my_dir in dirs:
            dir_path_tr = os.path.join(train_path_, my_dir)
            dir_path_te = os.path.join(test_path_, my_dir)
            # 训练集删除
